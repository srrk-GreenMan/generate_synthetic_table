...
사용되는 프롬프트는 `src/prompts/` 폴더에 분리되어 있어 다른 구성 요소에서 재사용하거나 쉽게 교체할 수 있습니다.
...
- `--save-json`: 요약/점검 정보를 JSON 파일로 저장합니다. HTML 표는 같은 이름을 가진 `_parsed.html`, `_synthetic.html` 파일로 별도 저장됩니다.

`main.py`는 내부의 `src.runner` 모듈을 사용해 동일한 로직을 실행하므로, 다른 스크립트나 노트북에서도 `run_flow_for_image` 함수를 가져다가 간단히 재사용할 수 있습니다.

`main.py`는 내부의 `src.runner` 모듈을 사용해 동일한 로직을 실행하므로, 다른 스크립트나 노트북에서도 `run_flow_for_image` 함수를 가져다가 간단히 재사용할 수 있습니다. 실행 결과는 HTML 파일(원본 구조, 합성 표)과 JSON(요약, 점검 결과, 파일 경로)으로 분리되어 저장됩니다.
